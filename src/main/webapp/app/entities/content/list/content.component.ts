...
import { ContentService, EntityArrayResponseType } from '../service/content.service';
...
export class ContentComponent implements OnInit {
  ...
  trackId = (item: IContent): number => this.contentService.getContentIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.contents.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IContent[] | null): IContent[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.contentService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
